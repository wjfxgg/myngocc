...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self,
        ...
        points = self.frustum.to(sensor2ego) - post_trans.view(B, N, 1, 1, 1, 3) #b n d h w 3
        ...
        # 这里设置一个检查点，检查这里的bda应该没有进行平移：
        assert torch.allclose(bda[:, :3, 3], torch.zeros_like(bda[:, :3, 3])), "bda should not have translation"
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        if stereo:
            ...
            #     # for stage in range(int(2)):
            #     cost_volumn_net.extend([
            #         nn.Conv2d(256, depth_channels, kernel_size=3,
            #                     stride=2, padding=1),
            #         nn.BatchNorm2d(depth_channels)])
            #     cost_volumn_net.extend([
            #         nn.Conv2d(88, depth_channels, kernel_size=3,
            #                     stride=2, padding=1),
            #         nn.BatchNorm2d(depth_channels)])
            # elif cost_volume_downsample == 2:
            #     cost_volumn_net.extend(
    ...
    def gen_grid(self, metas, B, N, D, H, W, hi, wi):
        ...
        combine = rots.matmul(torch.inverse(metas['intrins'][:, :, :3, :3]))

        points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points)
        points += trans.view(B, N, 1, 1, 1, 3, 1)
        neg_mask = points[..., 2, 0] < 1e-3
        points = metas['intrins'][:, :, :3, :3].view(B, N, 1, 1, 1, 3, 3).matmul(points)
    ...
    def calculate_cost_volumn(self, metas):
        ...
        end_time = time.time()
        # print(f"gen_grid time: {(end_time - start_time):.4f} s")
        ...
        end_time = time.time()
        # print(f"calculate_cost_volumn time: {(end_time - start_time):.4f} s")
        return cost_volumn

    def calculate_cost_volumn_fast(self, metas):
        prev, curr = metas['cv_feat_list']
        group_size = 4
        _, c, hf, wf = curr.shape
        hi, wi = hf * 4, wf * 4
    ...
    def calculate_cost_volumn_new(self, metas):
        prev, curr = metas['cv_feat_list']
        group_size = 16
        _, c, hf, wf = curr.shape
        hi, wi = hf * 4, wf * 4  # 保留原gen_grid所需的尺寸参数
        B, N, _ = metas['post_trans'].shape
        D, H, W, _ = metas['frustum'].shape
        start_time = time.time()
        grid = self.gen_grid(metas, B, N, D, H, W, hi, wi).to(curr.dtype)
        ...
        # 重塑为 [B*N, C, H, W]（与原逻辑一致）
        prev = prev.view(B * N, -1, H, W)
        curr = curr.view(B * N, -1, H, W)
        batch_size = B * N  # 合并后的批次大小
        num_channels = curr.shape[1]
        num_groups = num_channels // group_size  # 通道组数量（与原逻辑一致，忽略余数通道）
        start_time = time.time()
        # -------------------------- 核心优化：批量处理所有通道组 --------------------------
        if num_groups == 0:
            # 无有效通道组时，返回全0（与原循环不执行逻辑一致）
            return torch.zeros((batch_size, D, H, W), device=curr.device, dtype=curr.dtype)
        
        # 1. 通道分组：将 [B*N, C, H, W] → [B*N, num_groups, group_size, H, W]
        #    再转换为 [B*N*num_groups, group_size, H, W]（批量处理所有组）
        # prev_grouped = prev[:, :num_groups*group_size, ...].reshape(batch_size, num_groups, group_size, H, W)
        # curr_grouped = curr[:, :num_groups*group_size, ...].reshape(batch_size, num_groups, group_size, H, W)
        
        # 维度调整：合并 batch 和 group 维度，适配 grid_sample 批量处理
        # prev_batch = prev_grouped.permute(0, 1, 2, 3, 4).reshape(-1, group_size, H, W)  # [B*N*num_groups, group_size, H, W]
        # curr_batch = curr_grouped.permute(0, 1, 2, 3, 4).reshape(-1, group_size, H, W)  # [B*N*num_groups, group_size, H, W]

        # 2. 扩展 grid 维度，适配批量通道组（原grid: [B*N, D, H, W, 2] → 扩展为 [B*N*num_groups, D, H, W, 2]）
        grid_batch = grid.unsqueeze(1).repeat(1, num_groups, 1, 1, 1, 1).reshape(-1, D, H, W, 2)
        torch.backends.cudnn.enabled = False  # 禁用cuDNN
        wrap_prev = F.grid_sample(prev, grid, align_corners=True, padding_mode='zeros')
        torch.backends.cudnn.enabled = True
        # 3. 一次性 warp 所有通道组（替代原循环的多次 grid_sample）
        # wrap_prev = F.grid_sample(
        #     prev, 
        #     grid,
        #     align_corners=True,
        #     padding_mode='zeros'
        # )  # 输出形状：[B*N*num_groups, group_size, D, H, W]

        # 4. 批量计算代价体积（替代原循环的逐组累加）
        # 维度调整：恢复 batch 和 group 维度 → [B*N, num_groups, group_size, D, H, W]
        wrap_prev = wrap_prev.view(B*N, -1, D, H, W)
        # curr = curr.unsqueeze(2)  # 扩展 D 维度 → [B*N, num_groups, group_size, 1, H, W]（广播到D）
        
        # 计算差值→绝对值→通道组内求和（group_size维度）→所有组求和（num_groups维度）
        cost_volumn = (curr.unsqueeze(2) - wrap_prev).abs()  # [B*N, num_groups, group_size, D, H, W]
        cost_volumn = cost_volumn.sum(dim=2)  # 组内通道求和 → [B*N, num_groups, D, H, W]
        cost_volumn = cost_volumn.sum(dim=1)  # 所有组求和 → [B*N, D, H, W]（与原循环累加结果一致）

        # -------------------------- 保留原逻辑：处理bias和softmax --------------------------
        if self.bias != 0:
            # 保持原逻辑：用最后一个通道组的第一个通道判断无效值（与原循环最后一次迭代一致）
            last_group_wrap = wrap_prev[:, -1, 0, ...]  # [B*N, D, H, W]（最后一组的第一个通道）
            invalid = (last_group_wrap == 0)
            cost_volumn[invalid] += self.bias

        cost_volumn = -cost_volumn
        cost_volumn = cost_volumn.softmax(dim=1)  # D维度softmax（与原逻辑一致）
        end_time = time.time()
        # print(f"calculate_cost_volumn time: {(end_time - start_time):.4f} s")
        return cost_volumn
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        ...
        # end_time = time.time()
        # print('Calc cost volumn time: ', end_time - start_time)
        if self.with_cp:
