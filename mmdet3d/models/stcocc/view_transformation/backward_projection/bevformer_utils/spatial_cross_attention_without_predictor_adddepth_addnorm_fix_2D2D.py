...
class OA_SpatialCrossAttentionWithoutPredictor_adddepth_addnorm_fix_A_2D2D(BaseModule):
    ...
    def __init__(self,
        ...
        super(OA_SpatialCrossAttentionWithoutPredictor_adddepth_addnorm_fix_A_2D2D, self).__init__(init_cfg)
    ...
    def forward_layer(self,
        ...
        ego_dir_vector = kwargs['ego_dir_vector']
        # input check
        if key is None:
            key = query
        if value is None:
        ...
        unit_A = unit_A.view(bs,num_query,D,self.num_heads,self.num_levels,3).permute(0,1,3,4,2,5)
        ...
        ego_dir_vector = ego_dir_vector.flatten(2).permute(0, 2, 1)                     # [(bs n), h*w, C]
        ...
        unit_A_rebatch = unit_A.new_zeros([bs, self.num_cams, max_len, self.num_heads, self.num_levels, num_all_points,3])
        ...
        for i in range(bs):
            for j in range(num_cam):  
                ...
                unit_A_rebatch[i, j, :curr_numel] = unit_A[i, index_query_per_img]
        ...
        ego_dir_vector = ego_dir_vector.unsqueeze(2).contiguous()
        ...
        ego_dir_output = MultiScaleDeformableAttnFunction_fp32.apply(ego_dir_vector, spatial_shapes,level_start_index,depth_reference_points,depth_attention_weights, self.im2col_step)
        ...
        ego_dir_output = ego_dir_output.reshape(bs*n, max_len, D, -1)   # [bs*num_cam, num_query, num_Z_anchors, C]
        ...
        B = ego_dir_output[:,:,None,None]

        unit_A = unit_A_rebatch.reshape(bs*n,max_len,self.num_heads,self.num_levels,D,3)
        # sin_squard = (1-(unit_A*B).sum(-1)**2).view(bs,per_query_points,-1,self.num_heads,1,1).squeeze(-1).permute(0,2,3,4,1) # 2,625,8,1,1,2
        sin_squard_rebatch = (1-(unit_A*B).sum(-1)**2)
        # attention_weights = attention_weights *(sin_squard_rebatch.flatten(0,1))
        attention_weights = attention_weights *(sin_squard_rebatch)
...
@ATTENTION.register_module()
class OA_MSDeformableAttention3DWithoutPredictor_adddepth_addnorm_fix_A_2D2D(BaseModule):
