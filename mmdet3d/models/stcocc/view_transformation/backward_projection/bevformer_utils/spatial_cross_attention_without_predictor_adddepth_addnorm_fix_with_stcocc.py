...
class OA_SpatialCrossAttentionWithoutPredictor_adddepth_addnorm_fix_A_with_stcocc(BaseModule):
    ...
    def __init__(self,
        ...
        super(OA_SpatialCrossAttentionWithoutPredictor_adddepth_addnorm_fix_A_with_stcocc, self).__init__(init_cfg)
    ...
    def forward_layer(self,
        ...
        for i in range(bs):
            for j in range(num_cam):  
                ...
                reference_points_occlusion_rebatch[i, :curr_numel] = nonempty_voxel_logits[i, index_query_per_img]
        ...
        # occlusion_weights = depth_weights * (reference_points_occlusion_rebatch.flatten(2))
        occlusion_weights = ((depth_weights).view(bs,n,max_len,-1,1) * (reference_points_occlusion_rebatch[:,None])).squeeze(-1)
        occlusion_weights=occlusion_weights.flatten(0,1)
        attention_weights = attention_weights *(sin_squard_rebatch.flatten(0,1)) * occlusion_weights[:, :, None, None]
