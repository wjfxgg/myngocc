...
class OA_SpatialCrossAttentionWithoutPredictor_visibilitiy_v2(BaseModule):
    ...
    def __init__(self,
                 ...
                 deformable_attention=dict(
                     type='OA_MSDeformableAttention3DWithoutPredictor_visibility_v2',
                     embed_dims=256,
                     num_levels=4),
                 layer_scale=None,
                 dbound=None,
        ...
        super(OA_SpatialCrossAttentionWithoutPredictor_visibilitiy_v2, self).__init__(init_cfg)
...
@ATTENTION.register_module()
class OA_MSDeformableAttention3DWithoutPredictor_visibility_v2(BaseModule):
    ...
    def __init__(self,
                 ...
                 sigma=2,
                 dbound=None,
                 depth_score_mode='transmittance',
    ...
    def forward(self,
        ...
        def _sample_bins(bins: torch.Tensor):
            val0 = bins.gather(-1, depth_bin_idx0.unsqueeze(-1)).squeeze(-1)
            val1 = bins.gather(-1, depth_bin_idx1.unsqueeze(-1)).squeeze(-1)
            val_linear = val0
            if num_depth_bins > 1:
        ...
        num_all_points = attention_weights.size(-1)
        ...
        if do_debug_dump:
            ...
            if is_main:
                ...
                dump = {
                    ...
                    'depth_score_mode': self.depth_score_mode,
                    ...
                    'termination_bins': termination_bins[b0, q_idx].detach().cpu(),
                    'visibility_hard': visibility_hard[b0, q_idx].detach().cpu(),
                    'visibility_linear': visibility_linear[b0, q_idx].detach().cpu(),
                    'visibility_used': visibility_used[b0, q_idx].detach().cpu(),
                    'prob_hard': prob_hard[b0, q_idx].detach().cpu(),
                    'prob_linear': prob_linear[b0, q_idx].detach().cpu(),
                    'prob_used': prob_used[b0, q_idx].detach().cpu(),
                    'termination_hard': termination_hard[b0, q_idx].detach().cpu(),
                    'termination_linear': termination_linear[b0, q_idx].detach().cpu(),
                    'termination_used': termination_used[b0, q_idx].detach().cpu(),
                    'depth_score_used': depth_score[b0, q_idx].detach().cpu(),
                }
        ...
        if self.visibility_weight_mode == 'multiply':
            attention_weights = attention_weights * depth_score_points[:, :, None, None, :]
