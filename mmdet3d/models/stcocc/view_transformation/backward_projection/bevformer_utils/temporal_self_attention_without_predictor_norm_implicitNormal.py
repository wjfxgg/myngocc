...
class OA_TemporalAttentionWithoutPredictor_Norm_implicit(BaseModule):
    ...
    def __init__(self,
        ...
        self.dir_vector_mlp = nn.Sequential(
            nn.Linear(20*2*self.num_points, embed_dims),  # 20是bin的数量
            nn.ReLU(),
    ...
    def forward(self,
                ...
                ego_dir_vector_3d_xyz_i: Optional[torch.Tensor] = None, #bs,20(bin),z,h,w
                **kwargs) -> torch.Tensor:
        ...
        # 不再使用nonempty_voxel_logits计算attention weights
        ...
        if torch.rand(1) < 0.01:
            # sampling_offsets = sampling_offsets.flip(3)
            1==1
            pass
        ...
        # B = ego_dir_vector_3d_xyz_i.flatten(3).permute(0,3,2,1) #bs,num_query,num_points,20(x,y,z)
        # unit_A = A /  torch.linalg.vector_norm(A, dim=-1)[...,None]
        # sin_squard = (1-(unit_A*B[:,:,None,None,None]).sum(-1)**2)
        # unit_B = B /  torch.linalg.vector_norm(B, dim=-1)[...,None]
       ...
       # attention_weights = attention_weights * sin_squared
        # 不再使用occlusion-aware attention weights
    ...
    def sampling_forward(self,
        ...
        # 不再使用nonempty_voxel_logits进行采样
        # 直接使用所有点
        occlusion_mask = torch.ones((bs, num_query, 1), device=query.device, dtype=torch.bool)

        indexes = [[] for _ in range(bs)]
        max_len = 0
        for i in range(bs):
            index_query_per_batch = torch.arange(num_query, device=query.device)
        ...
        # 不再使用nonempty_voxel_logits
        sampling_offsets = self.sampling_offsets(queries_rebatch).view(bs, max_len, self.num_heads, self.num_bev_queue, self.num_levels, self.num_points, 2)
