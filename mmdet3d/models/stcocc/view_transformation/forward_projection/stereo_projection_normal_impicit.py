...
class LSSForwardProjectionWithNormal_implicit(LSSForwardProjection):
    """
...
@NECKS.register_module()
class LSSVStereoForwardProjectionWithNormal_implicit(LSSVStereoForwardPorjection):
    """
    ...
    def transform_normal_to_ego_with_bin_preservation(self, normal_preds, input):
        """
        将normal概率从相机坐标系变换到ego坐标系，同时保留分bin数据结构
        支持相机旋转和图像post增强的旋转变换
        ...
        Returns:
            dict: 包含以下内容的字典：
                - ego_w_angle: W方向角度（弧度制）
                - ego_h_angle: H方向角度（弧度制）
                - ego_d_angle: D方向角度（弧度制）
                - ego_dir_vector: ego坐标系归一化方向向量
                - ego_normal_comp_W_bins: ego坐标系下的W方向分bin概率
                - ego_normal_comp_H_bins: ego坐标系下的H方向分bin概率
                - ego_normal_comp_depth_bins: ego坐标系下的深度方向分bin概率
                - gt_depth: 真实深度值
                - cam_dir_vector: 相机坐标系原始方向向量
        """
        # 从input中解包参数（intrins保留但不再使用，避免修改输入解包逻辑）
        x, sensor2keyego, ego2global, intrins, post_rot, post_aug, bda, mlp_input = input[:8]
        
        # 获取概率分布
        normal_w_prob = normal_preds['normal_comp_W_bins'] # n bin_w h w
        normal_h_prob = normal_preds['normal_comp_H_bins']
        normal_depth_prob = normal_preds['normal_comp_depth_bins']
        ...
        # 计算bin中心角度（弧度制）
        w_angles_rad = torch.linspace(0, torch.pi-torch.pi/num_w_bins, num_w_bins, device=normal_w_prob.device) # w_bin
        h_angles_rad = torch.linspace(0, torch.pi-torch.pi/num_h_bins, num_h_bins, device=normal_w_prob.device)
        d_angles_rad = torch.linspace(0, torch.pi/2-torch.pi/2/num_d_bins, num_d_bins, device=normal_w_prob.device)
        
        # ========== 1. 对每个bin的中心角度进行旋转变换 ==========
        
        # 1.1 为所有W和H bin组合创建方向向量
        # 生成所有组合的W和H角度
        w_grid, h_grid = torch.meshgrid(w_angles_rad, h_angles_rad, indexing='ij') #w_bin h_bin
        w_flat = w_grid.reshape(-1)
        h_flat = h_grid.reshape(-1)
        
        # 将球面坐标转换为笛卡尔坐标（相机坐标系）
        sin_theta = torch.sin(h_flat)
        cos_theta = torch.cos(h_flat)
        sin_phi = torch.sin(w_flat)
        cos_phi = torch.cos(w_flat)
        ...
        # 形状：[num_w_bins*num_h_bins, 3]
        bin_dir_vectors = torch.stack([x_dir, y_dir, z_dir], dim=1)
        bin_dir_vectors = F.normalize(bin_dir_vectors, dim=1)
        
        # 1.2 应用旋转变换
        # 重塑方向向量以适应批量处理：[num_w_bins*num_h_bins, 3] -> [B*N*num_w_bins*num_h_bins, 3]
        bin_dir_vectors_batch = bin_dir_vectors.unsqueeze(0).repeat(B*N, 1, 1)  # [B*N, num_w_bins*num_h_bins, 3]
        bin_dir_vectors_batch = bin_dir_vectors_batch.transpose(1, 2)  # [B*N, 3, num_w_bins*num_h_bins]
        
        # 处理后旋转逆变换
        post_rot_inv = torch.inverse(post_rot).view(B*N, 3, 3)
        bin_dir_vectors_batch = torch.bmm(post_rot_inv, bin_dir_vectors_batch)
        
        # 应用sensor2keyego变换
        sensor2keyego_rot = sensor2keyego[:, :, :3, :3].reshape(B*N, 3, 3)
        bin_dir_vectors_batch = torch.bmm(sensor2keyego_rot, bin_dir_vectors_batch)
        
        # 应用BDD变换（可选）
        if bda is not None:
            bda_rot = bda[:, :3, :3].view(B, 1, 3, 3).expand(B, N, 3, 3).contiguous().view(B*N, 3, 3)
            bin_dir_vectors_batch = torch.bmm(bda_rot, bin_dir_vectors_batch)
        
        # 归一化变换后的方向向量
        bin_dir_vectors_batch = F.normalize(bin_dir_vectors_batch, dim=1)
        
        # 1.3 将变换后的方向向量转换回角度
        bin_dir_vectors_batch = bin_dir_vectors_batch.transpose(1, 2)  # [B*N, num_w_bins*num_h_bins, 3]
        
        # 极角theta (与z轴夹角) → ego_h_angle
        ego_z = bin_dir_vectors_batch[:, :, 2]
        ego_h_angles = torch.acos(torch.clamp(ego_z, -1.0, 1.0))  # [B*N, num_w_bins*num_h_bins]
        
        # 方位角phi (xy平面投影与x轴夹角) → ego_w_angle
        ego_x = bin_dir_vectors_batch[:, :, 0]
        ego_y = bin_dir_vectors_batch[:, :, 1]
        ego_w_angles = torch.atan2(ego_y, ego_x)  # [B*N, num_w_bins*num_h_bins]
        ego_w_angles = ego_w_angles % (2 * torch.pi)  # 归一化到0-2pi
        
        # ========== 2. 重新计算分bin概率 ==========
        
        # 2.1 初始化ego坐标系下的分bin概率
        ego_w_prob = torch.zeros_like(normal_w_prob)
        ego_h_prob = torch.zeros_like(normal_h_prob)
        ego_depth_prob = torch.zeros_like(normal_depth_prob)
        
        # 2.2 计算W方向分bin概率
        # 获取空间维度大小
        _, _, H, W = normal_w_prob.shape
        
        # 重塑角度为 [B*N, num_w_bins, num_h_bins, 1, 1] 以支持空间维度广播
        ego_w_angles_reshaped = ego_w_angles.view(B*N, num_w_bins, num_h_bins, 1, 1)
        
        for i in range(num_w_bins):
            # 计算当前bin中心角度与所有变换后角度的距离
            angle_dist = torch.abs(ego_w_angles_reshaped - w_angles_rad[i])
            angle_dist = torch.min(angle_dist, 2*torch.pi - angle_dist)  # 考虑角度的周期性
            
            # 计算权重（使用高斯核）
            weights = torch.exp(-angle_dist ** 2 / (2 * 0.1 ** 2))  # 标准差0.1弧度
            
            # 将权重归一化，确保每个空间位置的权重和为1
            weights = weights / (weights.sum(dim=1, keepdim=True) + 1e-8)
            
            # 计算每个原始W bin对当前ego W bin i的贡献
            # normal_w_prob形状: [B*N, num_w_bins, H, W]
            # weights形状: [B*N, num_w_bins, num_h_bins, 1, 1]
            contribution = torch.sum(normal_w_prob.unsqueeze(2) * weights, dim=1)
            # contribution形状: [B*N, num_h_bins, H, W]
            # 在num_h_bins维度上求和得到最终贡献
            contribution = torch.sum(contribution, dim=1)  # 形状变为[B*N, H, W]
            
            # 累加到ego W bin i
            ego_w_prob[:, i, :, :] = contribution
        
        # 2.3 计算H方向分bin概率（类似W方向）
        # 重塑角度为 [B*N, num_w_bins, num_h_bins, 1, 1] 以支持空间维度广播
        ego_h_angles_reshaped = ego_h_angles.view(B*N, num_w_bins, num_h_bins, 1, 1)
        
        for i in range(num_h_bins):
            # 计算当前bin中心角度与所有变换后角度的距离
            angle_dist = torch.abs(ego_h_angles_reshaped - h_angles_rad[i])
            angle_dist = torch.min(angle_dist, 2*torch.pi - angle_dist)  # 考虑角度的周期性
            ...
            # 计算每个原始H bin对当前ego H bin i的贡献
            # normal_h_prob形状: [B*N, num_h_bins, H, W]
            # weights形状: [B*N, num_w_bins, num_h_bins, 1, 1]
            contribution = torch.sum(normal_h_prob.unsqueeze(1) * weights, dim=2)
            # contribution形状: [B*N, num_w_bins, H, W]
            # 在num_w_bins维度上求和得到最终贡献
            contribution = torch.sum(contribution, dim=1)  # 形状变为[B*N, H, W]
            
            # 累加到ego H bin i
            ego_h_prob[:, i, :, :] = contribution
        
        # 2.4 深度方向处理（假设深度方向在旋转变换中保持不变，或简单复制）
        # 注意：深度方向的处理可能需要根据具体定义调整
        ego_depth_prob = normal_depth_prob
        
        # ========== 3. 计算期望角度和方向向量（与原方法相同） ==========
        
        # 计算期望角度
        w_expected_angle = torch.sum(ego_w_prob * w_angles_rad.view(1, -1, 1, 1), dim=1)
        h_expected_angle = torch.sum(ego_h_prob * h_angles_rad.view(1, -1, 1, 1), dim=1)
        d_expected_angle = torch.sum(ego_depth_prob * d_angles_rad.view(1, -1, 1, 1), dim=1)
        ...
        # 保存相机坐标系原始方向向量（使用期望角度计算）
        cam_sin_theta = torch.sin(torch.sum(normal_h_prob * h_angles_rad.view(1, -1, 1, 1), dim=1))
        cam_cos_theta = torch.cos(torch.sum(normal_h_prob * h_angles_rad.view(1, -1, 1, 1), dim=1))
        cam_sin_phi = torch.sin(torch.sum(normal_w_prob * w_angles_rad.view(1, -1, 1, 1), dim=1))
        cam_cos_phi = torch.cos(torch.sum(normal_w_prob * w_angles_rad.view(1, -1, 1, 1), dim=1))
        cam_x_dir = cam_sin_theta * cam_cos_phi
        cam_y_dir = cam_sin_theta * cam_sin_phi
        cam_z_dir = cam_cos_theta
        cam_dir_vector = torch.stack([cam_x_dir, cam_y_dir, cam_z_dir], dim=1)
        cam_dir_vector = F.normalize(cam_dir_vector, dim=1)
        ...
        ego_normal_results = {
            ...
            'ego_h_angle': ego_h_angle,  # H方向角度（弧度制）
            'ego_d_angle': ego_d_angle,  # D方向角度（弧度制）
            'ego_dir_vector': dir_vector,  # ego坐标系归一化方向向量
            'ego_normal_comp_W_bins': ego_w_prob,  # ego坐标系下的W方向分bin概率
            'ego_normal_comp_H_bins': ego_h_prob,  # ego坐标系下的H方向分bin概率
            'ego_normal_comp_depth_bins': ego_depth_prob,  # ego坐标系下的深度方向分bin概率
            'gt_depth': normal_preds.get('gt_depth', None),
            'cam_dir_vector': cam_dir_vector  # 相机坐标系原始方向向量
        }
    ...
    def forward(self, input, stereo_metas=None):
        ...
        if True:
            normal_preds.update(self.transform_normal_to_ego_with_bin_preservation(normal_preds, input))
        elif self.use_ego_2D_3D_trans_fix:
            normal_preds.update(self.transform_normal_to_ego_fix_remove_intrins_K(normal_preds, input))
        else:
        # 将normal概率从相机坐标系变换到ego坐标系
            normal_preds.update(self.transform_normal_to_ego(normal_preds, input))
        ...
        tran_feat_with_normal = torch.cat([tran_feat, normal_preds['ego_normal_comp_W_bins'],normal_preds['ego_normal_comp_H_bins'],normal_preds['ego_normal_comp_depth_bins']], dim=1)
        # 进行视图变换
        # 注意：这里使用拼接后的特征
        ...
        normal_preds['ego_dir_vector_3d'] = bev_feat[:, -20:, ...]
...
@NECKS.register_module()
class LSSVStereoForwardProjectionWithNormal_normalNet1_implicit(LSSVStereoForwardPorjection):
    """
    立体视觉版本的LSSForwardProjectionWithNormal
    继承了LSSVStereoForwardPorjection的所有功能，并添加了normal预测支持
    """
...
@NECKS.register_module()
class LSSVStereoForwardProjectionWithNormal_normalNet2_implicit(LSSVStereoForwardPorjection):
    """
    立体视觉版本的LSSForwardProjectionWithNormal
    继承了LSSVStereoForwardPorjection的所有功能，并添加了normal预测支持
    """
