...
class STCOccWithoutPredictor_normal_attn_addloss_implictNormal(CenterPoint):
    ...
    def create_multi_scale_features_fix(self,edg_feat_bev, scales=None):
        ...
        multi_scale_feats.append(F.interpolate(
                ...
                scale_factor=0.5,  # Z/X/Y各维度缩小2倍
                mode='area'  # 3D最近邻插值
            ))
        multi_scale_feats.append(F.interpolate(
                edg_feat_bev.float(),  # 转换为浮点类型
                scale_factor=0.25,  # Z/X/Y各维度缩小2倍
                mode='area'  # 3D最近邻插值
            ))
    ...
    def forward_train(self, points=None, img_metas=None, img_inputs=None, voxel_semantics=None, voxel_semantics_1_2=None, voxel_semantics_1_4=None, voxel_semantics_1_8=None, **kwargs):
        ...
        normal_comp_W_bins_pred,normal_comp_H_bins_pred,normal_comp_depth_bins_pred,ego_w_angle,ego_h_angle,ego_d_angle,ego_dir_vector,ego_normal_comp_W_bins,normal_comp_H_bins,normal_comp_depth_bins,_,cam_dir_vector,ego_dir_vector_3d,tran_feats = tran_feats.values()
        # ego_dir_vector_3d = torch.cat([ego_normal_comp_W_bins, normal_comp_H_bins, normal_comp_depth_bins], dim=1)
        # 预处理多尺度特征
