...
class GatedTemporalFusion6_cat_sparsefusion_my_cat(BaseModule):
    def __init__(
        self,
        history_num=4,
        ...
        **kwargs
        ...
        super(GatedTemporalFusion6_cat_sparsefusion_my_cat, self).__init__()
        ...
        self.history_fusion_linear = nn.Sequential(
            nn.Linear(single_bev_num_channels * (history_num + 1) + occ_embedims+(history_num+1)*6, single_bev_num_channels),
            nn.Softplus(),
            nn.Linear(single_bev_num_channels, single_bev_num_channels),
        )
        self.history_fusion_bg_linear = nn.Sequential(
            nn.Linear(single_bev_num_channels * (history_num//2 + 1) + occ_embedims+(history_num//2+1)*6, single_bev_num_channels),
            nn.Softplus(),
            nn.Linear(single_bev_num_channels, single_bev_num_channels),
        )
    ...
    def forward(self, curr_bev, cam_params, history_fusion_params, dx, bx, history_last_bev=None, last_occ_pred=None, nonempty_prob=None, img_feats=None, spatial_shapes=None,pred_img_depth=None,depth_bound=None,**kwargs):
        ...
        fg_fused = torch.cat([ fg_V_curr.view(bs,-1,self.top_k) , fg_curr_feat,fg_V_prev.view(bs,-1,self.top_k),fg_history_feat.view(bs, self.history_num, c_, self.top_k).reshape(bs, self.history_num*c_, self.top_k) , fg_occ_embed], dim=1).permute(0, 2, 1)
        ...
        bg_fused = torch.cat([ bg_V_curr.view(bs,6,-1) , bg_curr_feat,bg_V_prev.view(bs,-1,total_voxels-self.top_k),( bg_history_feat.view(bs, self.history_num//2, c_, total_voxels - self.top_k)).reshape(bs, self.history_num*c_//2, total_voxels - self.top_k), bg_occ_embed], dim=1).permute(0, 2, 1)
