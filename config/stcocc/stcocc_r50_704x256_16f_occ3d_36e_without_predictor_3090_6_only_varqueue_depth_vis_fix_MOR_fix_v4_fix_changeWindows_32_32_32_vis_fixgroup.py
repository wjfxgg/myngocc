...
# sequences first, then loads fra`mes from the sampled sequence in order
...
history_frame_num = [32, 32, 32]
model = dict(
    type='STCOccWithoutPredictor_VarQueueDepthVis_fix',
    num_stage=num_stage,
    bev_w=bev_h_,
    bev_h=bev_w_,
    bev_z=bev_z,
    ...
    forward_projection=dict(
        type='BEVDetStereoForwardProjection',  # 使用扩展的前向投影模块
        ...
        img_view_transformer=dict(
            type='LSSVStereoForwardPorjection',  # 使用扩展的视图变换器
            grid_config=grid_config,
            input_size=data_config['input_size'],
            in_channels=256,
            out_channels=forward_numC_Trans,
            # use_ego_2D_3D_trans_fix=True,
            sid=False,
            collapse_z=False,
    ...
    backward_projection=dict(
        ...
        transformer=dict(
            ...
            encoder=dict(
                # 使用不带predictor的encoder
                type='BEVFormerEncoderWithoutPredictor',
                num_layers=2,
                use_temporal=True,
                pc_range=point_cloud_range,
                ...
                transformerlayers=dict(
                    # 使用不带predictor的encoder layer
                    type='BEVFormerEncoderLayerWithoutPredictor',
                    attn_cfgs=[
                        dict(
                    ...
                    # 修改operation_order，移除predictor操作但保留conv操作
    ...
    temporal_fusion=dict(
        # MoR-style expert-choice routing: hard voxels get deeper/longer temporal fusion
        type='MoRQueueFusionWithoutPredictor_fix_vis_fixgroup',
        single_bev_num_channels=backward_numC_Trans,
        score_source='occ', # score_source: str = 'occ',  # 'occ' | 'feat'
        occ_score_mode='1-max', #  # 'entropy' | 'margin' | '1-max'
        free_index=occ_class_names.index('free'),
        # Optional: warm up from no dropping -> target capacity schedule to reduce early-training instability.
        # capacity_warmup_steps=5000,
        # queue_windows=None will default to (T/8, T/4, T/2, T) per stage
        queue_windows=(8, 16, 32),
        # capacity_factors=(1.0, 0.75, 0.5, 0.25),
        share_fusion = False,
        # debug_dump=True,
        # debug_dump_dir='work_dirs/mor_queue_debug',
        # debug_dump_interval=2000,
        # debug_dump_max=100,
        ...
        bev_z=bev_z,
        debug_dump=True,              # 开启
        debug_dump_dir='./mor_debug', # 保存路径
        debug_dump_interval=1,      # 每100个iter存一次
        debug_dump_max=5000,
    ),
    occupancy_head=dict(
        type='OccHead',
...
train_pipeline = [
    ...
    dict(type='PointToMultiViewDepth_fix_nomal_triview_fixdirectionByDepthBound_fixattn', downsample=1, grid_config=grid_config),
    dict(type='DefaultFormatBundle3D', class_names=occ_class_names),
    dict(type='Collect3D',
         keys=['img_inputs','normal_comp_W_bins','normal_comp_H_bins','normal_comp_depth_bins', 'gt_depth', 'voxel_semantics', 'voxel_semantics_1_2', 'voxel_semantics_1_4', 'voxel_semantics_1_8'])
]
...
share_data_config = dict(
    ...
    work_dir='stcocc_r50_704x256_16f_occ3d_36e_without_predictor',
    eval_show=True,
)
...
load_from = "work_dirs/stcocc_r50_704x256_16f_occ3d_36e_without_predictor_3090_6_only_varqueue_depth_vis_fix_MOR_fix_v4_fix_changeWindows_32_32_32/iter_103136_ema.pth"