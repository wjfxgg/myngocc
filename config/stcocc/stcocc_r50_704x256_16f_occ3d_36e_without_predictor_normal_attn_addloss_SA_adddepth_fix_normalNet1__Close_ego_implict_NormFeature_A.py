...
model = dict(
    type='STCOccWithoutPredictor_normal_attn_addloss',
    num_stage=num_stage,
    bev_w=bev_h_,
    bev_h=bev_w_,
    bev_z=bev_z,
    ...
    forward_projection=dict(
        type='BEVDetStereoForwardProjectionWithNormal_NormFeature',  # 使用扩展的前向投影模块
        ...
        img_view_transformer=dict(
            type='LSSVStereoForwardProjectionWithNormal_normalNet1_NormFeature_A',  # 使用扩展的视图变换器
        ...
        img_bev_encoder_backbone=dict(
            type='CustomResNet3D',
            numC_input=(forward_numC_Trans+3+20) * (len(range(*multi_adj_frame_id_cfg))+1),
            num_layer=[1, 2, 4],
            with_cp=False,
